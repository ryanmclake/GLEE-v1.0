...
diff <- w %>% select(country, biome_type, mean_diff, diff_sd, temp_for_model, lat) %>% na.omit(.) %>% filter(mean_diff >= 0) %>%
  mutate(temp_for_model = temp_for_model - 273.15)%>%
  mutate(temp_for_model = ifelse(temp_for_model < -100, NA, temp_for_model)) %>% mutate(sd = 0.1) %>%
  na.omit(.) %>%
...
temp_ebu_function_mcclure_mean <- function(x) {(36.77 * 1.436 ^ (x - 20))}
temp_ebu_function_mcclure_min <- function(x) {(9.061 * 1.339 ^ (x - 20))}
temp_ebu_function_mcclure_max <- function(x) {(66.657 * 1.547 ^ (x - 20))}
